<template>
  <div class="wrraper">
    ...
    <template>
      <!-- <Breadcrumb :canAddNew="false" :items="items" search_route="/orders/add" :search_title="$t('breadcrumb.orders.add')"
          icon="fa-plus" /> -->
...
export default {
  ...
  methods: {
    ...
    mapItem(data) {
      this.expandData = {
        [this.$t("labels.title") + this.$t("inArabic")]: data.service.title,
        [this.$t("labels.desc") + this.$t("inEnglish")]: data.service.desc,
    ...
    setRows() {
      let url = "";
      if (this.name === "users") {
        url = `clients-orders/${this.profileData.id}`;
      } else if (this.name === "callcenter") {
        url = `callcenter-orders/${this.profileData.id}`;
      } else if (this.name === "agents") {
        url = `agents/${this.profileData.id}/orders`;
      }
