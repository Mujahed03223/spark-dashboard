<template>
  ...
  <div v-else>
    ...
    <div class="form countryForm custom_card fadeIn" v-else>
      <ValidationObserver ref="policiesForm" @submit.prevent="submit">
        <div class="row">
          <!-- <UploadFiles
            @change="updateFiles"
...
export default {
  ...
  methods: {
    ...
    // Submit
    submit() {
      this.$refs.policiesForm.validate().then((d) => {
        if (d) {
          this.btnLoading = true;
          ...
          this.axios({
            ...
            .then(() => {
              ...
              this.$router.push("/privacy/show-all");
              this.btnLoading = false;
            })
            .catch((err) => {
              this.$iziToast.error({
