  ...
  <div class="wrraper">
    ...
    <template>
      <Breadcrumb
        :canAddNew="!canAdd"
        :items="items"
        search_route="/what_we_offers/add"
        :search_title="$t('breadcrumb.what_we_offers.add')"
        icon="fa-plus"
      />
          ...
          <main>
            ...
            >
              ...
              <template v-slot:[`item.name`]="{ item }">
                <router-link
                  v-if="item.full_name"
                  :to="`/what_we_offers/show/${item?.id}`"
                >
                  <span>
                    {{ item.full_name }}
                  </span>
              ...
              <template v-slot:[`item.desc_ar`]="{ item }">
                <span v-if="item.ar.desc" v-html="item.ar.desc"></span>
                <span class="redColor fontBold" v-else>{{
                  $t("notFound")
                }}</span>
              </template>
              <template v-slot:[`item.desc_en`]="{ item }">
                <span v-if="item.en.desc" v-html="item.en.desc"></span>
                <span class="redColor fontBold" v-else>{{
                  $t("notFound")
                }}</span>
              </template>
              ...
              <template v-slot:[`item.is_active`]="{ item }">
                <!-- <switcher :disabled="canEdit" name="what_we_offers" :item="item" value="is_active" type="active" /> -->
                <span
                  class="status pt-2"
              ...
              <template v-slot:top>
                <h3 class="table-title title">
                  {{ $t("breadcrumb.what_we_offers.title") }}
                  ...
                  <v-tooltip bottom>
                    ...
                    <span> {{ $t("breadcrumb.what_we_offers.all") }}</span>
                  </v-tooltip>
                </h3>
              </template>
              <!-- ======================== End Top Section ======================== -->
...
export default {
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t("breadcrumb.what_we_offers.title"),
          disabled: false,
          href: "/what_we_offers/show-all",
        },
        {
          text: this.$t("breadcrumb.what_we_offers.all"),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    showItem(item) {
      this.$router.push({ path: `/what_we_offers/show/` + item.id });
    },
    editItem(item) {
      this.$router.push({ path: `/what_we_offers/edit/` + item.id });
    },
  ...
  created() {
    ...
    this.canEdit = this.$permission.allowed("what_we_offers/edit/");
    this.canDelete = this.$permission.allowed("what_we_offers/delete");
    this.canAdd = this.$permission.allowed("what_we_offers/add");

    this.canShow = this.$permission.allowed("what_we_offers/show/");
    if (this.$route.query.page) {
      this.paginations.current_page = +this.$route.query.page;
    }
    this.setRows();
