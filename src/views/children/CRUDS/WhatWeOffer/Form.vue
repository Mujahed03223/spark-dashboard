<template>
  ...
  <div v-else>
    ...
    <div class="form countryForm custom_card fadeIn" v-else>
      <ValidationObserver ref="formwhat_we_offers" @submit.prevent="submit">
        <div class="row">
          <validation-provider
            v-slot="{ errors }"
  ...
  data() {
    return {
      items: [
        ...
        {
          text: this.$t(
            `breadcrumb.what_we_offers.${this.id ? "edit" : "add"}`
          ),
          disabled: true,
          href: "",
        },
  ...
  methods: {
    ...
    UploadFiles() {
      this.uploadedFiles.forEach((file) => {
        ...
        data.append("model", "what_we_offers");
    ...
    // Submit
    submit() {
      this.$refs.formwhat_we_offers.validate().then((d) => {
        if (d) {
          this.btnLoading = true;
          ...
          this.axios({
            ...
            .then(() => {
              ...
              this.$router.push("/what_we_offers/show-all");
              this.btnLoading = false;
            })
            .catch((err) => {
              this.$iziToast.error({
