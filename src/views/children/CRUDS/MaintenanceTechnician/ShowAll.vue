  ...
  <div class="wrraper">
    ...
    <template>
      <Breadcrumb
        :canAddNew="!canAdd"
        :items="items"
        search_route="/maintenanceTechnician/add"
        :search_title="$t('breadcrumb.providers.add')"
        icon="fa-plus"
      />
            ...
            >
              ...
              <template v-slot:[`item.phone`]="{ item }">
                <span v-if="item.phone"
                  ><bdi>{{ item.phone_code + item.phone }}</bdi></span
                >
                <span class="redColor fontBold" v-else>{{
                  $t("notFound")
                }}</span>
              ...
              <template v-slot:top>
                <h3 class="table-title title">
                  {{ $t("breadcrumb.maintenanceTechnician.title") }}
                  ...
                  <v-tooltip bottom>
                    ...
                    <span> {{ $t("breadcrumb.providers.all") }}</span>
                  </v-tooltip>
                </h3>
              </template>
              <!-- ======================== End Top Section ======================== -->
...
export default {
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t("breadcrumb.maintenanceTechnician.all"),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    showItem(item) {
      this.$router.push({ path: `/maintenanceTechnician/show/` + item.id });
    },
    editItem(item) {
      this.$router.push({ path: `/maintenanceTechnician/edit/` + item.id });
    },
  ...
  created() {
    ...
    this.canEdit = this.$permission.allowed("maintenanceTechnician/edit/");
    this.canDelete = this.$permission.allowed("maintenanceTechnician/delete");
    this.canAdd = this.$permission.allowed("maintenanceTechnician/add");

    this.canShow = this.$permission.allowed("maintenanceTechnician/show/");
    if (this.$route.query.page) {
      this.paginations.current_page = +this.$route.query.page;
    }
    this.setRows();
