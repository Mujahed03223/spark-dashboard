          ...
          <main>
            ...
            >
              ...
              <template v-slot:[`item.is_delayed`]="{ item }">
                <span
                  class="status"
                  :class="item.is_delayed ? 'active' : 'inactive'"
                >
              ...
              <template v-slot:[`item.status`]="{ item }">
                <span class="canceld">{{ $t(`status.${item.status}`) }}</span>
              </template>
              ...
              <template v-slot:[`item.extendIcon`]="{ item }">
                <div class="_actions">
                  <v-icon class="show" small @click="clickRow(item, 'service')">
                    fas fa-angle-double-down
                  </v-icon>
                </div>
              </template>

              <template v-slot:[`item.extendIcon2`]="{ item }">
                <div class="_actions">
                  <v-icon class="show" small @click="clickRow(item, 'cancel')">
                    fas fa-angle-double-down
                  </v-icon>
                </div>
              </template>
...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        {
          text: this.$t("labels.cancel_reason"),
          value: "extendIcon2",
          align: "center",
          sortable: false,
        },
  ...
  methods: {
    ...
    //     this.expandData = {
    //       [this.$t("labels.title")]:
    //         data.cancel_users[data.cancel_users.length - 1].user.full_name,
    //       [this.$t("labels.cancel_reason")]:
    //         data.cancel_users[data.cancel_users.length - 1].cancel_reason_note,
    //     };
    //   }
    // },
    mapItem(data) {
    ...
    clickRow(rowData, type) {
      ...
      if (indexExpanded > -1) {
        ...
        this.mapItem(rowData, this.type);
