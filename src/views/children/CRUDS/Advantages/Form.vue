<template>
  ...
  <div v-else>
    ...
    <div class="form countryForm custom_card fadeIn" v-else>
      <ValidationObserver ref="formadvantages" @submit.prevent="submit">
        <div class="row">
          <validation-provider
            v-slot="{ errors }"
...
export default {
  ...
  data() {
    return {
      items: [
        ...
        {
          text: this.$t(`breadcrumb.advantages.${this.id ? "edit" : "add"}`),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    // Submit
    submit() {
      this.$refs.formadvantages.validate().then((d) => {
        if (d) {
          this.btnLoading = true;
          ...
          this.axios({
            ...
            .then(() => {
              ...
              this.$router.push("/advantages/show-all");
              this.btnLoading = false;
            })
            .catch((err) => {
              this.$iziToast.error({
