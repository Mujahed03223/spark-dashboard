<template>
  ...
  <div v-else>
    ...
    <div class="form countryForm custom_card fadeIn" v-else>
      <ValidationObserver ref="featuresForm" @submit.prevent="submit">
        <div class="row">
          <validation-provider
            v-slot="{ errors }"
            :name="$t('labels.image')"
...
export default {
  ...
  data() {
    return {
      items: [
        ...
        {
          text: this.$t(`breadcrumb.features.${this.id ? "edit" : "add"}`),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    // Submit
    submit() {
      this.$refs.featuresForm.validate().then((d) => {
        if (d) {
          ...
          this.axios({
            ...
            .then(() => {
              ...
              this.$router.push("/features/show-all");
