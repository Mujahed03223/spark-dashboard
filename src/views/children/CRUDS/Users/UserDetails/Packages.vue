<template>
  <Loader v-if="loadingPackages"></Loader>
  <div v-else>
    <noData v-if="noData" />
        ...
        <div class="col-12">
          ...
          >
            ...
            <template v-slot:[`item.image`]="{ item, index }">
              <img
                class="image"
                :src="item.package && item.package.image"
                @error="imageError(index)"
                @click="show_model_1(item.package && item.package.image)"
                v-if="item.package && item.package.image"
              />
            ...
            <template v-slot:[`item.title`]="{ item }">
              <span v-if="item.package && item.package.title">
                {{ item.package.title }}
              </span>
              <span class="redColor fontBold" v-else>
                {{ $t("notFound") }}
            ...
            <template v-slot:[`item.auction_no`]="{ item }">
              <span v-if="item.package && item.package.auction_no">
                {{ item.package.auction_no }}
              </span>
              <span class="redColor fontBold" v-else>
                {{ $t("notFound") }}
              </span>
            ...
            <template v-slot:[`item.price`]="{ item }">
              <span v-if="item.package && item.package.price">
                {{ item.package.price }}
              </span>
              <span class="redColor fontBold" v-else>
                {{ $t("notFound") }}
            ...
            <template v-slot:[`item.status`]="{ item }">
              <span v-if="item.status" class="statuses" :class="item.status">
...
export default {
  ...
  methods: {
    imageError(index) {
      this.rows[
        index
      ].image = require("@/assets/media/images/Icons/no-data.svg");
    },
