<template>
  <div class="wrraper">
    ...
    <template>
      <Breadcrumb
        :items="items"
        search_route="/discounts/add"
        :search_title="$t('breadcrumb.discounts.add')"
        icon="fa-plus"
      />
        ...
        <div class="">
          ...
          <main>
            ...
            >
              ...
              <template v-slot:[`item.offer.image`]="{ item, index }">
                <img
                  @click="show_model_1(item.offer.image)"
                  class="image"
                  @error="imageError(index)"
                  :src="item.offer.image"
                  v-if="item.offer.image"
                />
              ...
              <template v-slot:[`item.actions`]="{ item }">
                <div class="_actions">
                  ...
                  <!-- <deleter v-if="!canDelete" :items="rows" name="discounts" :id="item.id" @refresh="rows = $event" /> -->
                </div>
              </template>
              ...
              <template v-slot:top>
                <h3 class="table-title title">
                  ...
                  <v-tooltip bottom>
                    ...
                    <span> {{ $t("breadcrumb.discounts.all") }}</span>
                  </v-tooltip>
                </h3>
              </template>
              <!-- ======================== End Top Section ======================== -->
...
export default {
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t("breadcrumb.discounts.title"),
          disabled: false,
          href: "/discounts/show-all",
        },
        {
          text: this.$t("breadcrumb.discounts.all"),
          disabled: true,
          href: "",
        },
      ],
      ...
      headers: [
        ...
        {
          text: this.$t("labels.title") + " " + this.$t("labels.provider"),
  ...
  methods: {
    ...
    showItem(item) {
      this.$router.push({ path: `/discounts/show/` + item.id });
    },
    editItem(item) {
      this.$router.push({ path: `/discounts/edit/` + item.id });
    },
  ...
  created() {
    ...
    this.canEdit = this.$permission.allowed("discounts/edit/");
    this.canDelete = this.$permission.allowed("discounts/delete");
    this.canAdd = this.$permission.allowed("discounts/add");

    this.canShow = this.$permission.allowed("discounts/show/");
    if (this.$route.query.page) {
      this.paginations.current_page = +this.$route.query.page;
    }
    this.setRows();
