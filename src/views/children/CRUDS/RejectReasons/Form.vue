<template>
  ...
  <div v-else>
    ...
    <div class="form countryForm custom_card fadeIn" v-else>
      <ValidationObserver ref="rejectForm" @submit.prevent="submit">
        <div class="row">
          <ValidationProvider
            v-slot="{ errors }"
        ...
        <div class="button_section d-flex gap-1 justify-content-center mt-10">
          ...
          <v-btn
            type="button"
            class="back_btn"
            @click="handleBack('/reject/show-all')"
          >
            {{ $t("back") }}
            <span
              :class="
...
export default {
  ...
  data() {
    return {
      items: [
        ...
        {
          text: this.$t(`breadcrumb.RejectReasons.${this.id ? "edit" : "add"}`),
          disabled: true,
          href: ""
        }
      ],
  ...
  methods: {
    ...
    // Submit
    submit() {
      this.$refs.rejectForm.validate().then((d) => {
        if (d) {
          ...
          this.axios({
            ...
            .then(() => {
              ...
              this.$router.push("/reject/show-all");
