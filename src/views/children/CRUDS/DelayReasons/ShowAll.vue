  ...
  <div class="wrraper">
    ...
    <template>
      <Breadcrumb
        :canAddNew="!canAdd"
        :items="items"
        search_route="/delayReasons/add"
        :search_title="$t('breadcrumb.delayReasons.add')"
        icon="fa-plus"
      />
          ...
          <main>
            ...
            >
              ...
              <!-- <template v-slot:[`item.name`]="{ item }">
                <router-link v-if="item.full_name" :to="`/delayReasons/show/${item?.id}`">
                  <span>
                    {{ item.full_name }}
                  </span>
                </router-link>
              ...
              <template v-slot:[`item.is_active`]="{ item }">
                <!-- <switcher :disabled="canEdit" name="delayReasons" :item="item" value="is_active" type="active" /> -->
                <span
                  class="status pt-2"
              ...
              <template v-slot:top>
                <h3 class="table-title title">
                  {{ $t("breadcrumb.delayReasons.title") }}
                  ...
                  <v-tooltip bottom>
                    ...
                    <span> {{ $t("breadcrumb.delayReasons.all") }}</span>
                  </v-tooltip>
                </h3>
              </template>
              <!-- ======================== End Top Section ======================== -->
...
export default {
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t("breadcrumb.delayReasons.all"),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    showItem(item) {
      this.$router.push({ path: `/delayReasons/show/` + item.id });
    },
    editItem(item) {
      this.$router.push({ path: `/delayReasons/edit/` + item.id });
    },
  ...
  created() {
    ...
    this.canEdit = this.$permission.allowed("delayReasons/edit/");
    this.canDelete = this.$permission.allowed("delayReasons/delete");
    this.canAdd = this.$permission.allowed("delayReasons/add");

    this.canShow = this.$permission.allowed("delayReasons/show/");
    if (this.$route.query.page) {
      this.paginations.current_page = +this.$route.query.page;
    }
    this.setRows();
