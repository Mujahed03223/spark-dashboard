  ...
  <div class="countries_all">
    <template>
      ...
      <Breadcrumb
        :canAddNew="!canAdd"
        :items="items"
        search_route="/steps/add"
        :search_title="$t('breadcrumb.steps.add')"
        icon="fa-plus"
      />
      <the-filter :inputs="inputs" />
...
export default {
  data() {
    return {
      items: [
        ...
        {
          text: this.$t("breadcrumb.steps.all"),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    setRows() {
      ...
      this.axios({
        ...
        .then((res) => {
          ...
          //           ? "/steps/show-all?is_active=1"
          //           : key == "deactivate_vehicle_count"
          //           ? "/steps/show-all?is_active=0"
          //           : key == "steps_count"
          //           ? "/steps/show-all"
          //           : "/steps/show-all?is_locked=1",
          //     });
          //   }
          // }
        })
    ...
    showItem(item) {
      this.$router.push({ path: `/steps/show/` + item.id });
    },
    editItem(item) {
      this.$router.push({ path: `/steps/edit/` + item.id });
    },
    fetchData(e) {
      this.$router.push({
  ...
  created() {
    this.canEdit = this.$permission.allowed("steps/edit/");
    this.canAdd = this.$permission.allowed("steps/add");
    this.canDelete = this.$permission.allowed("steps/delete");
    this.canShow = this.$permission.allowed("steps/show/");
    if (this.$route.query.page) {
      this.paginations.current_page = +this.$route.query.page;
    }
    this.setRows();
