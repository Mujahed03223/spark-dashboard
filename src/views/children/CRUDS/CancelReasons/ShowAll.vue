  ...
  <div class="wrraper">
    ...
    <template>
      <Breadcrumb
        :canAddNew="!canAdd"
        :items="items"
        search_route="/cancelReasons/add"
        :search_title="$t('breadcrumb.cancelReasons.add')"
        icon="fa-plus"
      />
          ...
          <main>
            ...
            >
              ...
              <!-- <template v-slot:[`item.name`]="{ item }">
                <router-link v-if="item.full_name" :to="`/cancelReasons/show/${item?.id}`">
                  <span>
                    {{ item.full_name }}
                  </span>
                </router-link>
              ...
              <template v-slot:[`item.is_active`]="{ item }">
                <!-- <switcher :disabled="canEdit" name="cancelReasons" :item="item" value="is_active" type="active" /> -->
                <span
                  class="status pt-2"
              ...
              <template v-slot:[`item.actions`]="{ item }">
                <div class="_actions">
                  <!-- <v-icon class="show" v-if="!canShow" small @click="showItem(item)">
                    fal fa-eye
                  </v-icon> -->
                  <v-icon
                    class="edit"
              ...
              <template v-slot:top>
                <h3 class="table-title title">
                  {{ $t("breadcrumb.cancelReasons.title") }}
                  ...
                  <v-tooltip bottom>
                    ...
                    <span> {{ $t("breadcrumb.cancelReasons.all") }}</span>
                  </v-tooltip>
                </h3>
              </template>
              <!-- ======================== End Top Section ======================== -->
...
export default {
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t("breadcrumb.cancelReasons.title"),
          disabled: false,
          href: "/cancelReasons/show-all",
        },
        {
          text: this.$t("breadcrumb.cancelReasons.all"),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    showItem(item) {
      this.$router.push({ path: `/cancelReasons/show/` + item.id });
    },
    editItem(item) {
      this.$router.push({ path: `/cancelReasons/edit/` + item.id });
    },
  ...
  created() {
    ...
    this.canEdit = this.$permission.allowed("cancelReasons/edit/");
    this.canDelete = this.$permission.allowed("cancelReasons/delete");
    this.canAdd = this.$permission.allowed("cancelReasons/add");

    this.canShow = this.$permission.allowed("cancelReasons/show/");
    if (this.$route.query.page) {
      this.paginations.current_page = +this.$route.query.page;
    }
    this.setRows();
