  ...
  <div class="wrraper">
    ...
    <template>
      <Breadcrumb
        :canAddNew="!canAdd"
        :items="items"
        search_route="/home_experts/add"
        :search_title="$t('breadcrumb.home_experts.add')"
        icon="fa-plus"
      />
          ...
          <main>
            ...
            >
              ...
              <template v-slot:[`item.is_active`]="{ item }">
                <!-- <switcher :disabled="canEdit" name="home_experts" :item="item" value="is_active" type="active" /> -->
                <span
                  class="status pt-2"
              ...
              <template v-slot:top>
                <h3 class="table-title title">
                  {{ $t("breadcrumb.home_experts.title") }}
                  ...
                  <v-tooltip bottom>
                    ...
                    <span> {{ $t("breadcrumb.home_experts.all") }}</span>
                  </v-tooltip>
                </h3>
              </template>
              <!-- ======================== End Top Section ======================== -->
...
export default {
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t("breadcrumb.home_experts.all"),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    showItem(item) {
      this.$router.push({ path: `/home_experts/show/` + item.id });
    },
    editItem(item) {
      this.$router.push({ path: `/home_experts/edit/` + item.id });
    },
  ...
  created() {
    ...
    this.canEdit = this.$permission.allowed("home_experts/edit/");
    this.canDelete = this.$permission.allowed("home_experts/delete");
    this.canAdd = this.$permission.allowed("home_experts/add");

    this.canShow = this.$permission.allowed("home_experts/show/");
    if (this.$route.query.page) {
      this.paginations.current_page = +this.$route.query.page;
    }
    this.setRows();
