  ...
  <div class="wrraper">
    ...
    <template>
      <Breadcrumb
        :canAddNew="!canAdd"
        :items="items"
        search_route="/sales/add"
        :search_title="$t('breadcrumb.sales.add')"
        icon="fa-plus"
      />
          ...
          <main>
            ...
            >
              ...
              <!-- <template v-slot:[`item.name`]="{ item }">
                <router-link v-if="item.full_name" :to="`/sales/show/${item?.id}`">
                  <span>
                    {{ item.full_name }}
                  </span>
                </router-link>
              ...
              <template v-slot:top>
                <h3 class="table-title title">
                  ...
                  <v-tooltip bottom>
                    ...
                    <span> {{ $t("breadcrumb.sales.all") }}</span>
                  </v-tooltip>
                </h3>
              </template>
              <!-- ======================== End Top Section ======================== -->
...
export default {
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t("breadcrumb.sales.all"),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    showItem(item) {
      this.$router.push({ path: `/sales/show/` + item.id });
    },
    editItem(item) {
  ...
  created() {
    ...
    this.canEdit = this.$permission.allowed("sales/edit/");
    this.canDelete = this.$permission.allowed("sales/delete");
    this.canAdd = this.$permission.allowed("sales/add");

    this.canShow = this.$permission.allowed("sales/show/");
    if (this.$route.query.page) {
      this.paginations.current_page = +this.$route.query.page;
    }
    this.setRows();
