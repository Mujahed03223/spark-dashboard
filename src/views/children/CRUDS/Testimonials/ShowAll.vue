  ...
  <div class="wrraper">
    ...
    <template>
      <Breadcrumb
        :canAddNew="!canAdd"
        :items="items"
        search_route="/testimonials/add"
        :search_title="$t('breadcrumb.testimonials.add')"
        icon="fa-plus"
      />
          ...
          <main>
            ...
            >
              ...
              <template v-slot:[`item.provider`]="{ item }">
                <router-link
                  v-if="item.provider?.full_name"
                  :to="`/providers/show/${item.provider?.id}`"
                >
                  <span>
              ...
              <template v-slot:[`item.job_ar`]="{ item }">
                <span v-if="item.ar.job" v-html="item.ar.job"></span>
                <span class="redColor fontBold" v-else>{{
                  $t("notFound")
                }}</span>
              </template>
              <template v-slot:[`item.job_en`]="{ item }">
                <span v-if="item.en.job" v-html="item.en.job"></span>
                <span class="redColor fontBold" v-else>{{
                  $t("notFound")
                }}</span>
              </template>
              ...
              <template v-slot:[`item.is_active`]="{ item }">
                <!-- <switcher :disabled="canEdit" name="testimonials" :item="item" value="is_active" type="active" /> -->
                <span
                  class="status pt-2"
              ...
              <template v-slot:top>
                <h3 class="table-title title">
                  {{ $t("breadcrumb.testimonials.title") }}
                  ...
                  <v-tooltip bottom>
                    ...
                    <span> {{ $t("breadcrumb.testimonials.all") }}</span>
                  </v-tooltip>
                </h3>
              </template>
              <!-- ======================== End Top Section ======================== -->
...
export default {
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t("breadcrumb.testimonials.title"),
          disabled: false,
          href: "/testimonials/show-all",
        },
        {
          text: this.$t("breadcrumb.testimonials.all"),
          disabled: true,
          href: "",
        },
      ],
      ...
      headers: [
        ...
        {
          text: this.$t("labels.job") + this.$t("inArabic"),
          value: "job_ar",
          align: "center",
          sortable: false,
        },
        {
          text: this.$t("labels.job") + this.$t("inEnglish"),
          value: "job_en",
          align: "center",
          sortable: false,
        },
  ...
  methods: {
    ...
    showItem(item) {
      this.$router.push({ path: `/testimonials/show/` + item.id });
    },
    editItem(item) {
      this.$router.push({ path: `/testimonials/edit/` + item.id });
    },
  ...
  created() {
    ...
    this.canEdit = this.$permission.allowed("testimonials/edit/");
    this.canDelete = this.$permission.allowed("testimonials/delete");
    this.canAdd = this.$permission.allowed("testimonials/add");

    this.canShow = this.$permission.allowed("testimonials/show/");
    if (this.$route.query.page) {
      this.paginations.current_page = +this.$route.query.page;
    }
    this.setRows();
