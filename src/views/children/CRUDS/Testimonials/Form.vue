<template>
  ...
  <div v-else>
    ...
    <div class="form countryForm custom_card fadeIn" v-else>
      <ValidationObserver ref="formtestimonials" @submit.prevent="submit">
        <div class="row">
          <validation-provider
            v-slot="{ errors }"
  ...
  data() {
    return {
      items: [
        ...
        {
          text: this.$t(`breadcrumb.testimonials.${this.id ? "edit" : "add"}`),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    UploadFiles() {
      this.uploadedFiles.forEach((file) => {
        ...
        data.append("model", "testimonials");
    ...
    // Submit
    submit() {
      this.$refs.formtestimonials.validate().then((d) => {
        if (d) {
          this.btnLoading = true;
          ...
          this.axios({
            ...
            .then(() => {
              ...
              this.$router.push("/testimonials/show-all");
              this.btnLoading = false;
            })
            .catch((err) => {
              this.$iziToast.error({
