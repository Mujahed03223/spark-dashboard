  ...
  <div class="wrraper">
    ...
    <template>
      <Breadcrumb
        :canAddNew="!canAdd"
        :items="items"
        search_route="/subService/add"
        :search_title="$t('breadcrumb.subService.add')"
        icon="fa-plus"
      />
          ...
          <main>
            ...
            >
              ...
              <template v-slot:[`item.is_active`]="{ item }">
                <!-- <switcher :disabled="canEdit" name="subService" :item="item" value="is_active" type="active" /> -->
                <span
                  class="status pt-2"
              ...
              <template v-slot:top>
                <h3 class="table-title title">
                  ...
                  <v-tooltip bottom>
                    ...
                    <span> {{ $t("breadcrumb.subService.all") }}</span>
                  </v-tooltip>
                </h3>
              </template>
              <!-- ======================== End Top Section ======================== -->
...
export default {
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t("breadcrumb.subService.all"),
          disabled: true,
          href: "",
        },
      ],
      ...
      headers: [
        ...
        {
          text: this.$t("labels.parent_service"),
          value: "service.title",
          align: "center",
          sortable: false,
        },
  ...
  methods: {
    ...
    showItem(item) {
      this.$router.push({ path: `/subService/show/` + item.id });
    },
    editItem(item) {
      this.$router.push({ path: `/subService/edit/` + item.id });
    },
  ...
  created() {
    ...
    this.canEdit = this.$permission.allowed("subService/edit/");
    this.canDelete = this.$permission.allowed("subService/delete");
    this.canAdd = this.$permission.allowed("subService/add");

    this.canShow = this.$permission.allowed("subService/show/");
    if (this.$route.query.page) {
      this.paginations.current_page = +this.$route.query.page;
    }
    this.setRows();
