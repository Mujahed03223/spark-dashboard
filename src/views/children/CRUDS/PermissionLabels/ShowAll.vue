<template>
  <div class="wrraper">
    ...
    <Breadcrumb
      :canAddNew="!canAdd"
      :items="items"
      search_route="/permission-labels/add"
      :search_title="$t('breadcrumb.permission-labels.add')"
      icon="fa-plus"
    />
    <!-- End Breadcrumb -->
      ...
      >
        ...
        <template v-slot:top>
          <h3 class="table-title title">
            {{ $t("breadcrumb.permission-labels.title") }}
            ...
            <v-tooltip bottom>
              ...
              <span>
                {{ $t("breadcrumb.permission-labels.all") }}
              </span>
            </v-tooltip>
          </h3>
        </template>
...
export default {
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t("breadcrumb.permission-labels.all"),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    // ==================== Start CRUD ====================
    addItem() {
      this.$router.push({ path: "/permission-labels/add" });
    },

    editItem(item) {
      this.$router.push({ path: "/permission-labels/edit/" + item.id });
    },
    // ===== Delete
  ...
  // ======= hooks
  created() {
    this.canEdit = this.$permission.allowed("permission-labels/edit/");
    this.canDelete = this.$permission.allowed("permission-labels/delete");
    this.canAdd = this.$permission.allowed("permission-labels/add");
    if (this.$route.query.page) {
      this.paginations.current_page = +this.$route.query.page;
    }
    // Set Rows
