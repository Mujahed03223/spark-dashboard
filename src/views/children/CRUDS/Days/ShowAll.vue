<template>
  <div class="wrraper">
    ...
    <template>
      <Breadcrumb
        :canAddNew="!canAdd && items.length > 7"
        :items="items"
        search_route="/days/add"
        :search_title="$t('breadcrumb.days.add')"
        icon="fa-plus"
      />
          ...
          <main>
            ...
            >
              ...
              <template v-slot:[`item.is_active`]="{ item }">
                <!-- <switcher :disabled="canEdit" name="days" :item="item" value="is_active" type="active" /> -->
                <span
                  class="status pt-2"
              ...
              <template v-slot:top>
                <h3 class="table-title title">
                  ...
                  <v-tooltip bottom>
                    ...
                    <span> {{ $t("breadcrumb.days.all") }}</span>
                  </v-tooltip>
                </h3>
              </template>
              <!-- ======================== End Top Section ======================== -->
...
export default {
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t("breadcrumb.days.all"),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    showItem(item) {
      this.$router.push({ path: `/days/show/` + item.id });
    },
    editItem(item) {
      this.$router.push({ path: `/days/edit/` + item.id });
    },
  ...
  created() {
    ...
    this.canEdit = this.$permission.allowed("days/edit/");
    this.canDelete = this.$permission.allowed("days/delete");
    this.canAdd = this.$permission.allowed("days/add");

    this.canShow = this.$permission.allowed("days/show/");
    if (this.$route.query.page) {
      this.paginations.current_page = +this.$route.query.page;
    }
    this.setRows();
