<template>
  ...
  <div v-else>
    ...
    <div class="form countryForm custom_card fadeIn" v-else>
      <ValidationObserver ref="formhome_services" @submit.prevent="submit">
        <div class="row">
          <validation-provider
            v-slot="{ errors }"
          ...
          <ValidationProvider
            v-slot="{ errors }"
            :name="$t('labels.sub_title') + $t('inArabic')"
            class="col-lg-6"
            rules="required|min:2|max:250"
          >
            <base-input
              :name="$t('labels.sub_title') + $t('inArabic')"
              type="text"
              :placeholder="$t('labels.sub_title') + $t('inArabic')"
              v-model="data.ar.sub_title"
            />
            <span v-if="errors[0]" class="error--text d-inline-block">
              {{ errors[0] }}
            </span>
          ...
          <ValidationProvider
            v-slot="{ errors }"
            :name="$t('labels.sub_title') + $t('inEnglish')"
            class="col-lg-6"
            rules="required|min:2|max:250"
          >
            <base-input
              :name="$t('labels.sub_title') + $t('inEnglish')"
              type="text"
              :placeholder="$t('labels.sub_title') + $t('inEnglish')"
              v-model="data.en.sub_title"
            />
            <span v-if="errors[0]" class="error--text d-inline-block">
              {{ errors[0] }}
            </span>
  ...
  data() {
    return {
      items: [
        ...
        {
          text: this.$t(`breadcrumb.home_services.${this.id ? "edit" : "add"}`),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    UploadFiles() {
      this.uploadedFiles.forEach((file) => {
        ...
        data.append("model", "home_services");
    ...
    getEditData() {
      ...
      this.axios({
        ...
        .then((res) => {
          ...
          this.data.ar.sub_title = result.ar.sub_title;
          this.data.en.sub_title = result.en.sub_title;
    ...
    // Submit
    submit() {
      this.$refs.formhome_services.validate().then((d) => {
        if (d) {
          this.btnLoading = true;
          ...
          data.append("ar[sub_title]", this.data.ar.sub_title);
          data.append("en[sub_title]", this.data.en.sub_title);
          ...
          this.axios({
            ...
            .then(() => {
              ...
              this.$router.push("/home_services/show-all");
              this.btnLoading = false;
            })
            .catch((err) => {
              this.$iziToast.error({
