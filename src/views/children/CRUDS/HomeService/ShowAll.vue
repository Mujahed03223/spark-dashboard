  ...
  <div class="wrraper">
    ...
    <template>
      <Breadcrumb
        :canAddNew="!canAdd"
        :items="items"
        search_route="/home_services/add"
        :search_title="$t('breadcrumb.home_services.add')"
        icon="fa-plus"
      />
          ...
          <main>
            ...
            >
              ...
              <template v-slot:[`item.is_active`]="{ item }">
                <!-- <switcher :disabled="canEdit" name="home_services" :item="item" value="is_active" type="active" /> -->
                <span
                  class="status pt-2"
              ...
              <template v-slot:top>
                <h3 class="table-title title">
                  {{ $t("breadcrumb.home_services.title") }}
                  ...
                  <v-tooltip bottom>
                    ...
                    <span> {{ $t("breadcrumb.home_services.all") }}</span>
                  </v-tooltip>
                </h3>
              </template>
              <!-- ======================== End Top Section ======================== -->
...
export default {
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t("breadcrumb.home_services.title"),
          disabled: false,
          href: "/home_services/show-all",
        },
        {
          text: this.$t("breadcrumb.home_services.all"),
          disabled: true,
          href: "",
        },
      ],
  ...
  methods: {
    ...
    showItem(item) {
      this.$router.push({ path: `/home_services/show/` + item.id });
    },
    editItem(item) {
      this.$router.push({ path: `/home_services/edit/` + item.id });
    },
  ...
  created() {
    ...
    this.canEdit = this.$permission.allowed("home_services/edit/");
    this.canDelete = this.$permission.allowed("home_services/delete");
    this.canAdd = this.$permission.allowed("home_services/add");

    this.canShow = this.$permission.allowed("home_services/show/");
    if (this.$route.query.page) {
      this.paginations.current_page = +this.$route.query.page;
    }
    this.setRows();
