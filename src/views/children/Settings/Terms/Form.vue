<template>
  ...
  <div v-else>
    ...
    <noData v-if="noData" />
    <div class="form termForm custom_card fadeIn" v-else>
      <ValidationObserver ref="formTerm" @submit.prevent="submit">
        <div class="row">
          <ValidationProvider
            v-slot="{ errors }"
...
export default {
  ...
  methods: {
    ...
    // Submit
    submit() {
      this.$refs.formTerm.validate().then((d) => {
        if (d) {
